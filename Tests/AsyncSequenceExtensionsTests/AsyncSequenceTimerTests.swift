...
final class AsyncTimerSequenceActorTests: XCTestCase {
    func testTimerPublisher() throws {
        ...
        Task {//(priority: .userInteractive) {
            ...
            let averageGap = gapsRelativeToTarget.reduce(0.0, +) / Double(gapsRelativeToTarget.count)
            print("Average relative time: \(averageGap)")
            XCTAssert((0.9...4).contains(averageGap))
            e.fulfill()
        }
        waitForExpectations(timeout: 3, handler: nil)
        _ = timerSequence
    ...
    func testTimerIterator() throws {
        ...
        Task {//(priority: .userInteractive) {
            ...
            let averageGap = gapsRelativeToTarget.reduce(0.0, +) / Double(gapsRelativeToTarget.count)
            print("Average relative time: \(averageGap)")
            XCTAssert((0.9...3).contains(averageGap))
            e.fulfill()
            _ = timerIterator
        }
        waitForExpectations(timeout: 3, handler: nil)
...
@available(iOS 15.0, macOS 12.0, *)
final class AsyncTimerSequenceUnreliableActorTests: XCTestCase {
    func testTimerPublisher() throws {
        let e = expectation(description: "Asynccomplete")
        let timerInterval = 0.001
        let timerSequence = AsyncTimerActorSequenceUnreliable(interval: timerInterval)
        Task {//(priority: .userInteractive) {
            let iterations = 30
            var deltas: [CFAbsoluteTime] = []
            deltas.reserveCapacity(iterations)
...
//        waitForExpectations(timeout: 3, handler: nil)
    ...
    func testTimerMap() throws {
        let itemsToExpect = 5
        var counter = 0
        let asyncSequence = AsyncTimerActorSequenceUnreliable(interval: 0.1)
            .prefix(itemsToExpect)
            .map { _ -> Int in
                counter += 1
                return counter
